...
Use **UC semantics** when the mart is already chosen and you need one ARR/NRR formula for every dashboard. Use **Neo4j SL** when the job is *finding* `a_1007` among opaque names and decoys. They stack: Neo4j retrieves the table; a Metric View can still own the formula. Details in [findings](eval/findings.md#databricks-unity-catalog-semantics-vs-neo4j-sl).
...
```bash
uv sync
cp .env.example .env        # warehouse, Neo4j, API keys — never commit .env
...
No OpenAI (or over quota)? Run embeddings locally:
...
The benchmark pre-flights providers and prints **over quota** instead of hanging. `seed_lakehouse.py --dump-sql` writes [`sql/lakehouse.sql`](sql/lakehouse.sql) without a warehouse. Do not ingest `samples.tpcds_sf1` — those catalogs often lack `information_schema.columns`.
...
`.env` is gitignored. [`.env.example`](.env.example) is placeholders only. Never commit PATs, Neo4j passwords, workspace URLs, or API keys.
